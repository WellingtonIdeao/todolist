...
class TodoItemListViewTestCase(APITestCase):
    ...
    def test_list_all_todo_items_response_data(self):
        TodoItem.objects.create(**self.data)
        queryset = TodoItem.objects.all()
        serializer = TodoItemSerializer(instance=queryset, many=True)
        response = self.client.get(self.url)
        self.assertEqual(response.data, serializer.data)
        
    def test_create_one_todo_item_status_201(self):
        response = self.client.post(self.url, self.data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    
    def test_create_one_todo_item_response_data(self):
        response = self.client.post(self.url, self.data)
        instance = TodoItem.objects.get(pk=1)
        serializer = TodoItemSerializer(instance)
        self.assertEqual(response.data, serializer.data)
...
class TodoItemDetailViewTestCase(APITestCase):
    ...
    def test_get_todo_item_by_id_response_data(self):
        response = self.client.get(self.url)
    ...
    def test_todo_item_update_response_data(self):
        full_data = { 
            'title': 'title updated',
            'description': 'description updated',
            'done': True
    ...
    def test_todo_partial_update_response_data(self):    
        partial_data = { 
            'title': 'title updated',
        }
        response = self.client.patch(self.url, partial_data)
